...
export default function Dashboard() {
  ...
  async function loadData() {
    try {
      ...
      for (const p of projs) {
        try {
          sm[p.id] = await listSprints(p.id)
        } catch {
          sm[p.id] = []
        }
      }
  ...
  const totalTickets = Object.values(sprintsMap).reduce(
    (sum, s) => sum + s.reduce((t, sp) => t + sp.total_tickets, 0),
    0,
  )
  const activeSprints = Object.values(sprintsMap).reduce(
    (sum, s) => sum + s.filter((sp) => sp.state === 'active').length,
    0,
  )
          ...
          {projects.map((p) => {
            ...
            return (
              <div key={p.id} style={styles.card}>
                ...
                <div style={styles.cardText}>密钥: {p.jira_project_key}</div>
                <div style={styles.cardText}>Sprint 数: {sprints.length}</div>
                <div style={{ marginTop: '0.5rem' }}>
                  {sprints.slice(-3).map((sp) => (
                    <div
                      key={sp.id}
                      style={styles.sprintItem}
                      onClick={() => navigate(`/sprint/${sp.id}`)}
                    >
                      <span>{sp.name}</span>
        ...
        <div style={styles.modalOverlay} onClick={() => setImportProject(null)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <input
              style={styles.input}
              type="text"
              placeholder="输入 Sprint 名称（如: LPRO Sprint 0707）"
              value={sprintName}
